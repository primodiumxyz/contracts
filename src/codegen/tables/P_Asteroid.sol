...
library P_Asteroid {
  ...
  function decodeStatic(bytes memory _blob) internal pure returns (int32 xBounds, int32 yBounds) {
    xBounds = (int32(uint32(Bytes.getBytes4(_blob, 0))));

    yBounds = (int32(uint32(Bytes.getBytes4(_blob, 4))));
  }
   ...
   */
  function encode(int32 xBounds, int32 yBounds) internal pure returns (bytes memory, EncodedLengths, bytes memory) {
    bytes memory _staticData = encodeStatic(xBounds, yBounds);
