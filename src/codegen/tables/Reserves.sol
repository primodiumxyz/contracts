...
library Reserves {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "Primodium", name: "Reserves", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x74625072696d6f6469756d000000000052657365727665730000000000000000);
  ...
  function decodeStatic(bytes memory _blob) internal pure returns (uint256 amountA, uint256 amountB) {
    amountA = (uint256(Bytes.getBytes32(_blob, 0)));

    amountB = (uint256(Bytes.getBytes32(_blob, 32)));
  }
   ...
   */
  function encode(uint256 amountA, uint256 amountB) internal pure returns (bytes memory, EncodedLengths, bytes memory) {
    bytes memory _staticData = encodeStatic(amountA, amountB);
