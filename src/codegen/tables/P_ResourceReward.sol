...
struct P_ResourceRewardData {
  uint8[] resources;
  uint256[] amounts;
}

library P_ResourceReward {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "Primodium", name: "P_ResourceReward", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x74625072696d6f6469756d0000000000505f5265736f75726365526577617264);
   ...
   */
  function get(bytes32 prototype) internal view returns (P_ResourceRewardData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = prototype;
   ...
   * @notice Get the full data.
   */
  function _get(bytes32 prototype) internal view returns (P_ResourceRewardData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = prototype;
   ...
   */
  function set(bytes32 prototype, P_ResourceRewardData memory _table) internal {
    bytes memory _staticData;
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(bytes32 prototype, P_ResourceRewardData memory _table) internal {
    bytes memory _staticData;
    EncodedLengths _encodedLengths = encodeLengths(_table.resources, _table.amounts);
    bytes memory _dynamicData = encodeDynamic(_table.resources, _table.amounts);
  ...
  ) internal pure returns (P_ResourceRewardData memory _table) {
