...
struct Meta_UtilityMapData {
  bool stored;
  uint256 index;
}

library Meta_UtilityMap {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "Primodium", name: "Meta_UtilityMap", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x74625072696d6f6469756d00000000004d6574615f5574696c6974794d617000);
   ...
   */
  function get(bytes32 entity, uint8 utility) internal view returns (Meta_UtilityMapData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](2);
    _keyTuple[0] = entity;
    _keyTuple[1] = bytes32(uint256(utility));
   ...
   * @notice Get the full data.
   */
  function _get(bytes32 entity, uint8 utility) internal view returns (Meta_UtilityMapData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](2);
    _keyTuple[0] = entity;
    _keyTuple[1] = bytes32(uint256(utility));
   ...
   */
  function set(bytes32 entity, uint8 utility, Meta_UtilityMapData memory _table) internal {
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(bytes32 entity, uint8 utility, Meta_UtilityMapData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.stored, _table.index);
  ...
  function decode(
    ...
    EncodedLengths,
    bytes memory
  ) internal pure returns (Meta_UtilityMapData memory _table) {
