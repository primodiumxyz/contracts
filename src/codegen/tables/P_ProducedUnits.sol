...
struct P_ProducedUnitsData {
  bytes32[] units;
  uint256[] amounts;
}

library P_ProducedUnits {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "Primodium", name: "P_ProducedUnits", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x74625072696d6f6469756d0000000000505f50726f6475636564556e69747300);
   ...
   */
  function get(bytes32 prototype) internal view returns (P_ProducedUnitsData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = prototype;
   ...
   * @notice Get the full data.
   */
  function _get(bytes32 prototype) internal view returns (P_ProducedUnitsData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = prototype;
   ...
   */
  function set(bytes32 prototype, P_ProducedUnitsData memory _table) internal {
    bytes memory _staticData;
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(bytes32 prototype, P_ProducedUnitsData memory _table) internal {
    bytes memory _staticData;
    EncodedLengths _encodedLengths = encodeLengths(_table.units, _table.amounts);
    bytes memory _dynamicData = encodeDynamic(_table.units, _table.amounts);
  ...
  ) internal pure returns (P_ProducedUnitsData memory _table) {
